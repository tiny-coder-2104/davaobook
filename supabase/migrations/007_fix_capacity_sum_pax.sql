-- ============================================================================
-- DavaoBook — 007: capacity check must SUM pax, not count booking ROWS
-- Bug (QA davaobook-0035): 002_create_booking_fn.sql did
--     SELECT count(*) INTO v_booked ...
--     IF v_booked + p_pax > v_capacity
-- which mixes units — booking ROWS on the left, PAX capacity on the right.
-- Repro on live: family-cabin capacity_per_day=3 accepted three bookings of
-- pax 2 + 1 + 1 (4 pax stored) because count(*) = 3.
--
-- Fix: COALESCE(SUM(pax), 0) over the SAME active-status set.
--
-- Replaces: create_booking_transactional (defined in 002_create_booking_fn.sql).
-- No later migration redefined it — 003_fix_booking_code_empty_name.sql only
-- replaces generate_booking_code(), 003_account_cols/004/005/006 only touch
-- operators columns, the signup trigger and the verified badge.
--
-- Callers: BOTH POST /api/bookings (public) and POST /api/admin/bookings/manual
-- call supabaseAdmin.rpc("create_booking_transactional", ...), so this single
-- replacement fixes the public, manual and CONFIRMED-capacity paths at once.
-- ============================================================================
...
BEGIN
  ...
  --    Sum PAX of active bookings for this package+tour_date
  --    (007: was count(*) — rows vs pax mixed units, see header)
  ...
  --    Check capacity: existing pax + requested pax must fit
