...
- 5.4 `curl -s -w "\n%{http_code}" "$U/api/track?email=$EMAIL"` → 200, array; each item has **exactly 7 keys**: `code, status, tour_date, end_date, package_name, pax, total_amount` (migration 010 added `end_date` — exclusive checkout date; equals `tour_date + 1` for single-night bookings). Assert no `email`, `guest_name`, `mobile`, `pickup_area`, `notes` keys.
